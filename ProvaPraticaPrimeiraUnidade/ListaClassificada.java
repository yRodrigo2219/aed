public class ListaClassificada<T>{
    ...
    //Metodo da prova adicionado!
    public ListaClassificada<T> invert(){
        if(isEmpty()){
            System.err.println("Lista vazia!");
            return this;
        }

        ListaClassificada<T> newList = new ListaClassificada<>();
        newList.add(this.tail.getData());

        Element<T> prev = this.head;

        for(Element<T> ptr = this.tail; ptr != this.head;){
            while(prev != null && prev.getNext() != ptr){
                prev = prev.getNext();
            }
            
            newList.add(prev.getData());
            ptr = prev;
            prev = this.head;
        }

        //    O metodo tem um retorno pois nao eh possivel re-declarar o
        // proprio objeto dentro dele mesmo. Ex: this = newList; Isso eh
        // um comando invalido.
        return newList;
    }
